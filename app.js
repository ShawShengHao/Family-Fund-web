...
if (chart) {
  ...
  chart.addEventListener('pointermove', (event) => { const rect=svg.getBoundingClientRect(), x=((event.clientX-rect.left)/rect.width)*700, actual=window.fundChartPoints||points, point=actual.reduce((a,b)=>Math.abs(b.x-x)<Math.abs(a.x-x)?b:a), gainClass=Number.parseFloat(point.gain)<0?'negative':'positive'; marker.setAttribute('cx',point.x); marker.setAttribute('cy',point.y); guide.setAttribute('x1',point.x); guide.setAttribute('x2',point.x); tooltip.innerHTML=`<span>${point.date}</span><strong>淨值 ${point.nav}</strong><b class="return-value ${gainClass}">${point.gain}</b>`; tooltip.style.left=`${Math.max(8,Math.min(82,(point.x/700)*100))}%`; chart.classList.add('is-hovered'); });
